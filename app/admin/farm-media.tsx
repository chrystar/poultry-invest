...
import { deleteFarmMedia, updateFarmMedia, uploadFarmMediaFile, useFarmMedia, type FarmMedia } from '../../hooks/useFarmMedia';

export default function AdminFarmMediaScreen() {
  const { user } = useAuth();
  const { items, loading, refetch } = useFarmMedia({ includeUnpublished: true });
  const [uploading, setUploading] = useState(false);
  const [editingItem, setEditingItem] = useState<FarmMedia | null>(null);
  ...
  const handleTogglePublish = async (item: FarmMedia) => {
    try {
      await updateFarmMedia(item.id, { is_published: !item.is_published });
      refetch();
    } catch (err: any) {
  ...
  const handleDelete = (item: FarmMedia) => {
    Alert.alert('Delete media', 'This removes the file and its record permanently. Continue?', [
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Farm Operations</Text>
      </View>
          ...
          renderItem={({ item }) => (
            <View style={[styles.row, shadow.card]}>
              <Pressable onPress={() => setEditingItem(item)}>
                <Image source={{ uri: item.public_url }} style={styles.thumb} contentFit="cover" />
              ...
              <Pressable style={{ flex: 1 }} onPress={() => setEditingItem(item)}>
                ...
                <Text style={styles.rowMeta} numberOfLines={1}>
                  {item.media_type === 'video' ? 'Video' : 'Photo'} · {new Date(item.created_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'short' })}
                </Text>
              </Pressable>
...
function EditMediaSheet({ item, onClose, onSaved }: { item: FarmMedia; onClose: () => void; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateFarmMedia(item.id, { title, caption });
      onSaved();
    } catch (err: any) {
  ...
  return (
    <View style={styles.sheetOverlay}>
      ...
      <View style={styles.sheet}>
        ...
        <InputField label="Title" value={title} onChangeText={setTitle} placeholder="e.g. Morning feeding round" />
        <InputField
          label="Caption"
          value={caption}
